...
public class GLRenderContext implements RenderContext {
	...
	public void display(GLAutoDrawable drawable) {
		...
		while (iterator.hasNext()) {
			...
			if (r.getShape() != null) {
				if (!sceneManager.getFrustum().shouldCull(r))
					draw(r);
				else
					culled++;
			}
...
//				System.out.print("Could not get location of uniform variable " + lightString + "\n");
			...
			if(iter != null) {
				...
				// put multiple light sources into an array
				while(iter.hasNext() && nLights<8)
				{
					...
					id = gl.glGetUniformLocation(activeShaderID, lightString);
					if(id!=-1)
						gl.glUniform4f(id, l.position.x, l.position.y, l.position.z, 0.f);		// Set light direction
					else
						System.out.print("Could not get position of uniform variable " + lightString + "\n");
					
					lightString = "cl[" + nLights + "]";			
					id = gl.glGetUniformLocation(activeShaderID, lightString);
					if(id!=-1)
						gl.glUniform3f(id, l.diffuse.x, l.diffuse.y, l.diffuse.z);		// Set light direction
					else
						System.out.print("Could not get color of uniform variable " + lightString + "\n");
